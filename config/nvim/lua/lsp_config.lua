...
-- Resolve at load so Vue TS integration tracks the active toolchain
-- (npm global, volta/nvm shim, etc.) instead of a hardcoded user path.
local vue_ls_bin = vim.fn.exepath("vue-language-server")
if vue_ls_bin == "" then
  vue_ls_bin = "vue-language-server"
end
...
function M.setup_autocommands()
  ...
  -- fidget.nvim already surfaces progress; LspDetach/LspProgress prints
  -- only cause hit-enter prompts with cmdheight=0, so no handlers here.

  -- Format-on-save lives in conform.nvim (formatters_by_ft), not here.
end
